...
bool NQueens::solve(std::string mode){
    if(mode == "column") {
        if (solve_rec_col(0) == false) {
...
bool NQueens::solve_rec_col(int row){
    //If all queens are placed then return true
    if (row >= board_size){ return true; }

    // Attempt to place a Queen in every column in this
    // row, one at a time.
    for (int i = 0; i < board_size; i++) {
        // Can the Queen safely be placed here?

        if (is_valid_col(row, i)) {
            // Place the Queen
            (*this->board)[row][i] = 1;
            // Recursively move to the next column
            // If it returns 1, then a Queen was succesfully
            // placed on that column as well
            if (this->solve_rec_col(row + 1)){ return true; }

            // If we did not return in the above statement,
            // we need to remove the queen from the current column
            // and place it on the next row
            (*this->board)[row][i] = 0; // BACKTRACK
        }
    }
...
bool NQueens::is_valid_col(unsigned int row, unsigned int col){
    int i, j;

    /* Check above this col */
    for (i = 0; i < row; i++)
    ...
    for (i = row, j = col; i >= 0 && j < this->board_size; i--, j++)
        if ((*this->board)[i][j]) { return false; }
