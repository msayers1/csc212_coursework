...
Maze::Maze(int rows, int cols, unsigned int seed, std::string filename ){
    ...
    std::vector<std::vector<std::vector<double>>>* grid_data_pointer = &grid_data;
    ...
    while(!current_stack.empty()){
        ...
        if(!neighbors.empty()){
            ...
            if(!current_stack.empty()) current_cell = current_stack.back();
        }
