...
bool AdjList::isAdjacent(unsigned int source, unsigned int destination){
    std::list<std::pair<unsigned int, unsigned int>>* adjListSource = &adjList[source];
    for(std::list<std::pair<unsigned int, unsigned int>>::iterator it = adjListSource->begin(); it != adjListSource->end(); ++it){
...
void AdjList::problem1(std::string fname){
	...
	for(unsigned int i = 0; i < this->adjList.size(); i++){
		for(std::list<std::pair<unsigned int, unsigned int>>::iterator it = this->adjList[i].begin(); it != this->adjList[i].end(); ++it){
			of << "\t" << i << " -> " << (*it).first << "[label=\"" << it->second << " \"] [color=\"" << ((it->second%2 == 1)?"red":"blue") << "\"]" << ";" << std::endl;
		}
	}
...
void AdjList::DFS(std::ofstream& of, unsigned int& vertex, bool visited[], int depth){
    ...
    for(std::list<std::pair<unsigned int, unsigned int>>::iterator it = this->adjList[vertex].begin(); it != this->adjList[vertex].end(); ++it){
			of << "\t" << vertex << " -> " << (*it).first << "[label=\"" << it->second << " \"] [color=\"" << colors[depth] << "\"]" << ";" << std::endl;
            this->DFS(of, (*it).first, visited, depth + 1);
    }
    // Your code here!
